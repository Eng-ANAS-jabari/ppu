...
<body class="p-4 md:p-6">
    ...
    <div id="app" class="max-w-7xl mx-auto space-y-6">
        ...
        <div id="roleSelection" class="bg-white p-12 rounded-[3rem] shadow-2xl text-center no-print border border-slate-200">
            <div class="mb-8">
                <span class="bg-indigo-100 text-indigo-700 px-4 py-1 rounded-full text-sm font-bold">الإصدار 2.1 - متصل ببياناتك</span>
    ...
    <script>
        // 🔗 الرابط المباشر من جدولك
        const SHEET_ID = '1Ne4jRjMj75t2zk-w9bCoO2jZlkJkOIxtQAlqNG91p3U';
        const SHEET_URL = `https://docs.google.com/spreadsheets/d/${SHEET_ID}/gviz/tq?tqx=out:csv`;
        
        // ⚠️ ضع رابط نشر Google Apps Script هنا إذا أردت الحفظ السحابي
        ...
        async function fetchSheetData() {
            ...
            try {
                ...
                console.log('تم جلب البيانات بنجاح، عدد الأحرف:', csvData.length);
                ...
                Papa.parse(csvData, {
                    ...
                    complete: (results) => {
                        ...
                        raw.forEach((row, index) => {
                            console.log(`تحليل الصف ${index}:`, row);
                            ...
                            if(pName && sName) {
                                ...
                                console.warn(`بيانات غير مكتملة في الصف ${index}:`, row);
                            }
                        });
                        
                        db = Object.values(projects);
                        console.log('المشاريع المجمعة:', db);
                        localStorage.setItem('grad_db_v3', JSON.stringify(db));
                        updateAdminStats();
                        updateCharts();
                        showNotification(`تم تحديث بيانات ${db.length} مجموعة بنجاح!`, 'success');
                    },
                    error: (error) => {
                        console.error('خطأ في تحليل CSV:', error);
                        showNotification('خطأ في تحليل البيانات. تأكد من تنسيق الجدول', 'error');
                    }
                });
            } catch (err) {
                console.error('خطأ في جلب البيانات:', err);
                showNotification('خطأ في جلب البيانات من السحابة. تأكد من إعدادات المشاركة', 'error');
            } finally {
                showLoading(false);
            }
        }
        ...
        function setRole(role) {
            if(db.length === 0) {
                ...
                if(local) {
                    db = JSON.parse(local);
                    showNotification('تم تحميل البيانات المحفوظة محلياً', 'info');
                } else {
                    showNotification('الرجاء تحديث البيانات أولاً باستخدام زر "تحديث البيانات"', 'error');
                    return;
                }
            }
            ...
            sel.innerHTML = '<option value="">-- اختر المجموعة --</option>' + 
                db.map(p => `
                    <option value="${p.groupId}|${p.title}">
                        ${p.room} | مجموعة ${p.groupId} - ${p.title.substring(0, 30)}${p.title.length > 30 ? '...' : ''}
                    </option>
                `).join('');
        }
        ...
        function saveToLocal() {
            ...
            showNotification('تم الحفظ المحلي تلقائياً', 'info');
        }

        async function saveToCloud() {
            if(!APPS_SCRIPT_URL) {
                showNotification('لم يتم ضبط رابط الحفظ السحابي. تم الحفظ محلياً فقط.', 'info');
                saveToLocal();
                return;
            }
